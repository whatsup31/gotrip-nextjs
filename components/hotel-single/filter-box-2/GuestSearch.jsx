// components/hotel-single/filter-box-2/GuestSearch.jsx
"use client";
      ...
      <div className="row y-gap-10 justify-between items-center">
        ...
        <div className="col-auto">
          <div className="d-flex items-center js-counter">
            <button className="button -outline-blue-1 text-blue-1 size-38 rounded-4" onClick={dec}>
              <i className="icon-minus text-12" />
            </button>
            <div className="flex-center size-20 ml-15 mr-15">
              <div className="text-15">{value}</div>
            </div>
            <button className="button -outline-blue-1 text-blue-1 size-38 rounded-4" onClick={inc}>
...
export default function GuestSearch({
  ...
  useEffect(() => {
    if (
      ...
      setState({ adults, children, rooms });
    }
    ...
    <div className="searchMenu-guests px-30 lg:py-20 sm:px-20 js-form-dd js-form-counters position-relative">
      ...
      <div className="shadow-2 dropdown-menu min-width-400">
        <div className="bg-white px-30 py-30 rounded-4 counter-box">
          <Counter label="Adults" value={state.adults} onChange={(v) => update("adults", v)} />
          <Counter label="Children" value={state.children} onChange={(v) => update("children", v)} />
          <Counter label="Rooms" value={state.rooms} onChange={(v) => update("rooms", v)} />
        </div>
      </div>
    </div>
  );
