...
export default async function HostBookingsPage() {
  ...
  // Récupération des listings du host
  ...
  if (lerr) {
    return <div className="p-6 text-red-600">{lerr.message}</div>;
  }

  const listingIds = (myListings ?? []).map(l => l.id);

  // Récupération des réservations liées
  let bookings: any[] = [];
        ...
        {bookings.map(b => (
          <li key={b.id} className="border rounded p-3">
            <div className="font-medium">
              {b.listings?.title} — {b.listings?.location}
            </div>
            <div className="text-sm">
              From {b.start_date} to {b.end_date} • {b.nights} nights
            </div>
            <div className="text-sm">
              Total: €{b.total_price} • Status: {b.status}
            </div>
            <div className="text-xs opacity-60">
              #{b.id} • created {new Date(b.created_at).toLocaleString()}
            </div>
          </li>
        ))}
      </ul>
